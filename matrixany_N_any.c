...
int main(int argc, char *argv[])	{
	...
	int rank, P, blksz;
	int lastblk;  // size of last block
	...
	blksz = N/P;
	lastblk = N%P;  // finding the reminder 
	...
	if(rank == 0) {
		// last block is handeled by master over here
		...
		printf("size of blksz is : %d\n", blksz);
		printf("size of last block is : %d\n", lastblk);
