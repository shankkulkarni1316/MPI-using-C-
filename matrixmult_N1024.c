...
int main(int argc, char *argv[])	{
	...
	static double  E[N][N], E1[N][N];  // new buffer values for scatter and gather
	double time1, time2;
	double time3, s; 		// for timing and speed-up
	...
	if (rank == 0) {
		...
		time1 = MPI_Wtime();       // start of sequential multiplication
		...
		time2 = MPI_Wtime(); 	// end of sequential multiplication
	}
	//MPI_Barrier(MPI_COMM_WORLD);
	
	MPI_Scatter(A, blksz*N, MPI_DOUBLE, E, blksz*N, MPI_DOUBLE, 0, MPI_COMM_WORLD);// Scatter input matrix A
	...
	MPI_Gather(E1, blksz*N, MPI_DOUBLE, C, blksz*N, MPI_DOUBLE, 0, MPI_COMM_WORLD);

	if(rank == 0) {
		
		time3 = MPI_Wtime(); 	// record  time stamp
		...
		printf("elapsed_time sequential = %f (seconds)\n", time2 - time1);  // print out execution time
		printf("elapsed_time parallel = %f (seconds)\n", time3 - time2);  // print out execution time
		s=(time2-time1)/(time3-time2);
		printf("speed-up: %f\n",s);
	}
	MPI_Finalize(); 
	return 0;
